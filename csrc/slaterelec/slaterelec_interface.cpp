...
// coupled-solve matvec), the first-order VJPs of both, and the energy's HVP (double backward); the Slater multipolar Pauli repulsion with the same three ops. See csrc/slaterelec/slaterelec.cu.
TORCH_LIBRARY_FRAGMENT(torchff, m) {
    ...
    m.def("slater_pauli_pair_energy(Tensor coords, Tensor pairs, Tensor b, Tensor a_i, Tensor a_j) -> Tensor");
    m.def("slater_pauli_pair_grad(Tensor coords, Tensor pairs, Tensor b, Tensor a_i, Tensor a_j, Tensor g) -> (Tensor, Tensor, Tensor, Tensor)");
    m.def("slater_pauli_pair_hvp(Tensor coords, Tensor pairs, Tensor b, Tensor a_i, Tensor a_j, Tensor g, Tensor v_coords, Tensor v_b, Tensor v_ai, Tensor v_aj) -> (Tensor, Tensor, Tensor, Tensor, Tensor)");
}
